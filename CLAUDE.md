...
エクスプローラは **Tree / Icons / List** の表示モードを切替可能（QSettings に永続化）。
Icons / List は現在フォルダの中身を表示し、先頭に「..（上の階層）」を置く
（Icons=大アイコン、List=サムネイル＋項目名を横並びの行）。全モードでコンテキスト
メニュー操作は共通。Icons/List は `_fill_flat()` で同じ内容を並べる。
