...
### ダウンロード導線の方針（Phase 4.5 で再設計）

- **フォルダ階層は実ディスクのディレクトリ**を主軸とする（論理フォルダではない）。
  ツリーはライブラリルート配下の実ディレクトリを走査して表示し、絞り込みは
  `rel_path` の前方一致で行う。横断分類は**タグ**で補う（ハイブリッド）。
- ダウンロードは**フォルダを右クリック →「ここにダウンロード」→ ポップアップ
  （URL＋形式）→ キューに投入**という導線にする。ファイルはそのフォルダに
  実際に保存され、自動登録される。
- **ダウンロードキューは順次処理（1件ずつ）**。専用の **Queue タブ**（進捗一覧・
  キャンセル）を設け、従来の Download タブは置き換える。
- 論理フォルダ（DB `folders`）は UI から外す（スキーマは互換のため残置）。
