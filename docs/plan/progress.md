...
- **フェーズ**: Phase 6（仕上げ）完了。主要機能・UIは一通り実装済み。
- **直近の仕上げ**: 空状態の案内、ワーカーのエラー処理、README 刷新、実ライブラリ
  "Youtube"（Violin/4本・非ASCII/サブフォルダ・最大1.3GB）での通し確認（scan/missing OK）。
- **レイアウト**: Library タブ = **[左：エクスプローラ（唯一のブラウザ）｜右上：
  プレイヤー｜右下：詳細]**。エクスプローラに実フォルダ＋ファイルを表示し、再生・
  D&D移動・削除・改名・複製・タグ付与を右クリック/キーで実行。タグ/欠落でツリー絞り込み。
- **UI 像**: ~1920×1080 で エクスプローラ／再生／詳細 を一画面。
- **エクスプローラ表示モード**: Tree / Icons を切替可能（永続化）。Icons は現在フォルダの
  中身を大アイコンで表示し、先頭に「..（上の階層）」。サムネイルがあれば表示。
- **永続化**: ウィンドウ位置・ペイン分割・詳細開閉・表示モード・フォルダ開閉。
...
- 2026-06-28: **Phase 6 完了（仕上げ）**。
  ...
  - **README を全面刷新**（現行のエクスプローラ/プレイヤー/詳細・DLキュー構成に）。
  - **実ライブラリ "Youtube" で通し確認**: `Violin/` 配下 4 本（非ASCIIファイル名・
    最大 1.3GB）を scan、missing 0。Unicode/サブフォルダ/大容量を実データで確認。
  - 全13テストスイート通過。
...
- [x] **Phase 6**: 仕上げ（空状態の案内、ワーカーのエラー処理、README 刷新、
      実ライブラリ "Youtube" での通し確認）。UI 拡張: エクスプローラ Tree/Icons 切替、
      D&D移動・ツリー削除、詳細の折りたたみ、クリックシーク、UI状態の永続化。
