...
- 原因: `QVideoWidget` に子ウィジェットで字幕を重ねると、実動画のネイティブ描画面に
  隠れて字幕が見えなかった（字幕テキスト自体は正しく算出されていた）。
- 修正: 動画描画を `QGraphicsView` + `QGraphicsVideoItem` に変更し、字幕を同一シーン上の
  テキストアイテムとして合成。確実に動画の上へ重なるようにした。
- 実クリップ（VP9 4K・全角文字を含む長尺）で字幕アイテムが表示されることを確認。

## 2026-06-29（字幕トグル＋サイドカー .srt 自動検出）

- `ui/player_widget.py`: **字幕トグルボタン**を追加（.srt があるときだけ有効）。
  表示/非表示を切替でき、状態は QSettings に永続化。
- `ui/main_window.py`: DB に字幕パスが無くても、動画と同じフォルダの
  `<stem>.srt` / `<stem>.<lang>.srt` を**自動検出**して表示（`.en.srt` 優先）。

## 2026-06-29（フォルダの削除）

- `core/library.py`: `delete_dir`（実フォルダごとゴミ箱へ。配下クリップを DB から
  除外し、クリップ/マーカーのサムネイルキャッシュも整理）。ルート削除は拒否。
  `_remove_path` をディレクトリ対応に。
- `ui/filter_panel.py`: フォルダ右クリックに「Delete folder...」（件数を出して確認）。

## 2026-06-28（動画クリックで再生/一時停止）

- `ui/player_widget.py`: 動画エリアの左クリックで再生/一時停止をトグル
  （メディア未ロード時は無視）。

## 2026-06-28（フォルダごとのドラッグ移動）

- `core/library.py`: `move_dir`（実フォルダを別フォルダ配下へ移動し、配下クリップの
  rel_path/subtitle_path を追従）。自身/子孫への移動・同名衝突・ルート移動を拒否。
  改名/移動の共通処理を `_reprefix_clips` に整理。
- `ui/filter_panel.py`: ツリーで**サブフォルダをドラッグ&ドロップ移動**できるように
  （フォルダに DragEnabled を付与、`folder_dropped` を処理）。
- 各ケース（サブツリー移動・子孫拒否・ルート移動・no-op・衝突）をテスト。
